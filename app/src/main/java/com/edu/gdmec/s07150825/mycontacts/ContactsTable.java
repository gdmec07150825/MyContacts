    ...
    public ContactsTable(Context context) {
        ...
        if (!db.isTableExits(TABLENAME)) {
            String createTableSql = "CREATE TABLE IF NOT EXISTS " + TABLENAME + "(id_DB integer "
                    + "primary key AUTOINCREMENT," + User.NAME + " Varchar," + User.MOBILE + " " +
                    "VARCHAR," + User.QQ + " VARCHAR," + User.DANWEI + " VARCHAR," + User.ADDRESS
                    + " VARCHAR)";
    ...
    public User getUserByID(int id) {
        ...
        try {
            cursor = db.find("select * from " + TABLENAME + "where " + " id_DB=?", new
                    String[]{id + ""});
            cursor.moveToNext();
            temp.setId_DB(cursor.getInt(cursor.getColumnIndex("di_DB")));
